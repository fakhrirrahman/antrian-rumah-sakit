...
@section('title', 'Ambil Nomor Antrian Farmasi')

@section('content')
<div class="max-w-md mx-auto bg-white p-8 rounded-lg shadow-lg mt-10">
    <h2 class="text-2xl font-bold text-center text-blue-600 mb-6">Ambil Nomor Antrian Farmasi</h2>
    @if(session('success'))
    <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
        {{ session('success') }}
    </div>
    @endif
    <form action="{{ route('ambil-antrian-farmasi.store') }}" method="POST" class="space-y-6">
        @csrf
        <div>
